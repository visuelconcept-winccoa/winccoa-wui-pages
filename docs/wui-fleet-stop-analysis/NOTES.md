...
- Access from the **Machine Fleet 3D** view: the "Analyse des causes d'arrêts" (stop-cause analysis) button of the overview (`mf-atelier-overview`) emits `wui:analyze`, the `wui-machine-fleet-3d` shell triggers `RouterEvent('/fleet-stops')`. The page's Back button returns to `RouterEvent('/fleet-3d')`.
- **This is where the stop-cause catalog editor lives** (moved from the overview): toolbar button "Causes d'arrêt" (stop causes) (icon `alarm`) → `mf-stop-causes` dialog (`.store` / `.canEdit` / `@wui:close`). On close, the page reloads the catalog (`listStopCauses`) and **recomputes** the analysis, because the causes' classifications/labels affect the result. This is the main page of the "stop causes" function.
...
- Unknown / off-catalog codes → folded onto the catalog's `isDefault` entry (same logic as `formatStopCause`). With no default and no code → "Sans cause assignée" (no cause assigned).
...
- **Runtime prerequisite — NGA archiving**: the state/cause DPEs must be NGA-archived, otherwise `dpGetPeriod` returns nothing and the page shows "Aucune donnée d'historique" (no history data). MachineSim DPs are **not** archived by default → enable via the per-machine archiving toggle (`FleetStore.setArchive`) or the NGA config.
