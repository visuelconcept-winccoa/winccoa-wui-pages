...
5. deploys the **`rtspProxy` manager** → `<projet>/javascript/rtspProxy/` + `npm install` (ffmpeg-static, rtsp-relay); with `--register-pmon`, adds the line to `config/progs`;
6. runs **`build:pages`** (OUT_DIR=`<projet>/data/dashboard-wc`).

## After install (mandatory)
1. **Webserver**: `cd <projet>/javascript/customer-webserver && npm run build`, then **restart** the webserver manager (it auto-mounts `/api/rtsp` + the `/api/rtsp/ws` relay).
...
## Verify
1. Logged in → **"Flux caméras (RTSP)"** entry, `/camera-streams` loads the list.
