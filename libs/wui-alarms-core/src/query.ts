...
import type { Alarm, Severity } from './types.js';
...
/** Rows of the snapshot the source keeps: standing only, or everything. */
export function inSource(alarm: Alarm, source: AlarmSource): boolean {
  return source === 'active' ? alarm.cleared === null : true;
}
...
export function applyQuery(all: readonly Alarm[], query: AlarmQuery, now: number): AlarmPage {
  ...
  const bands = query.severities && query.severities.length > 0 ? new Set<Severity>(query.severities) : null;

  const filtered = inSnapshot.filter((alarm) => {
    if (!inScope(alarm, query.scope)) return false;
    if (bands && !bands.has(alarm.severity)) return false;
