...
export class EngController {
  ...
  /**
   * POST /api/eng/books/:id/refresh — re-qualify the stored book. A source
   * REGENERATION (re-browse, re-ingest) is a separate operation: for file-based
   * books it is `/books/ingest`, which replaces the stored book.
   */
  public refreshBook = (req: Request, res: Response): void => {
    const book = this.store.readBook<AddressBook>(String(req.params['id']));
    if (book === null) {
  ...
  public ingestBook = (req: Request, res: Response): void => {
    ...
    if (!body.bookId || !body.format) {
      res.status(400).json({ ok: false, error: 'bookId and format (simaticml | xvm | csv) are required' });
      return;
    }
    try {
