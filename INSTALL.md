...
```bash
node webserver/install.mjs --project <PROJET> [--winccoa <install-WinCCOA>] [--register-pmon]
```
...
standard s'il tourne. (Détails : `webserver/SETUP.md`.)
...
pré-buildé d'une autre version ne fonctionne pas). `packages/` est généré
(`node tools/build-package.mjs tools/specs.json`, ou récupéré d'une Release). Pour para :
```bash
node packages/wui-para/install.mjs --workspace <WORKSPACE-RUNTIME> --project <PROJET>
```
...
(Détails par page : `docs/wui-<page>/INTEGRATION.md`.)
...
Le **kit partagé est vendorisé** dans le package (`_vendor/<lib>/`) → pas de prérequis kit/fleet-core/ai-kit séparé.

---

## Packages de page disponibles
Chaque dossier `packages/wui-<page>/` est un module source autonome (kit vendorisé), installé
par `node packages/wui-<page>/install.mjs --workspace <ws> --project <projet> [--register-pmon]`.
...
- `specs.json` — déclaration des 15 packages.

Regénérer tout : `node tools/build-package.mjs tools/specs.json`.
